import { Order } from "../../../Database/Models/CommonModel/OrderSchema.js"; //get info for single order by admin 
export const getSingleOrderByadmin = async (req, res) => {
    try {
        ...
        const order = await Order.findOne({ _id: orderId, sellerId: sellerId, }).populate("user", "name email");
...
export const allOrdersByAdmin = async (req, res) => {
    try {
        ...
        if (!orders || orders.length === 0) {
            return res.status(404).json({
                success: false, message: "No Orders Found for the User",
            });
        }
